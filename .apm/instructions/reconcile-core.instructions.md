...
| `profile/drumline/` | Illini Drumline domain profile: `migrations/0002_drumline_outreach.sql`, `migrate.py` (core+profile migrations), `import_drumline.py`, `name_resolutions.py`, `import_master.py`, `export_members.py`, `config.py` (PII configs in git-ignored `var/drumline/`). Single CLI: `python -m reconcile_core.profile.drumline <migrate\|import-drumline\|import-master\|name-resolutions\|export-members>`. Refresh order: migrations → import-drumline → import-master → name-resolutions → export-members (import-master overwrites the outreach overlay). |
