...
uv run python -m reconcile_core duplicates                    # suggest duplicate entities
uv run python -m reconcile_core merge 12 7                    # fold entity 12 into entity 7
...
2. **Resolve** — platform IDs resolve to store entities via `external_refs` (the Google `resourceName` is one more ref); likely duplicates are surfaced by `duplicates` and folded losslessly with `merge` (or undone with `split`)
