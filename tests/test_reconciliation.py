...
def test_reconciliation_new_data():
    reconciler = Reconciler()
    proposed = StandardContact(source_id="p1", display_name="John Doe", emails=["new@example.com"])
    current = StandardContact(source_id="people/1", display_name="John Doe", emails=["old@example.com"])
...
def test_phones_all_existing():
    ...
    reconciler = Reconciler()
    proposed = StandardContact(source_id="p1", display_name="Jane", phones=["+1 555-0100"])
    current = StandardContact(source_id="people/1", display_name="Jane", phones=["+1-555-0100"])
    diff = reconciler.reconcile(proposed, current)
...
def test_no_changes():
    ...
    reconciler = Reconciler()
    c = StandardContact(source_id="p1", display_name="Bob", emails=["bob@test.com"], phones=["555-0100"])
    current = StandardContact(source_id="people/1", display_name="Bob", emails=["bob@test.com"], phones=["555-0100"])
